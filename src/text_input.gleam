...
type KeyMap = dict.Dict(key.Key, String)
...
pub fn update(model: Model, event) {
    let #(model, event) = case event {
        event.Key(key) -> case result.unwrap(dict.get(model.key_map, key), "") {
            "character_forward" -> #(character_forward(model), event)
            "character_backward" -> #(character_backward(model), event)
            "delete_character_backward" -> #(delete_character_backward(model), event)
            "" -> {
                let model = case key {
                    key.Char(char) -> {
...
fn delete_character_backward(model: Model) {
    case string.length(model.value) > 0 {
        True -> {
            let value = string.slice(model.value, 0, int.max(0, model.position - 1))
...
fn insert_character(model: Model, char) {
    ...
    let tail = string.slice(model.value, model.position, string.length(model.value))
...
pub fn view(model: Model) {
  let value = string.slice(model.value, model.offset, model.offset_right - model.offset)
  let position = int.max(0, model.position - model.offset)
  let view = string.slice(value, 0, position)
  
  model.prompt <> case position < string.length(value) {
        True -> {
            let char = string.slice(value, position, 1)
            let c = cursor.set_char(model.cursor, char)
            view <> cursor.view(c) <> string.slice(value, position + 1, string.length(value))
        }
        False -> {
